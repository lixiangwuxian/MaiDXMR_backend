...
VideoEncoderNVENC::VideoEncoderNVENC(std::shared_ptr<CD3DRender> pD3DRender,
    ...
    : m_pD3DRender(pD3DRender), m_codec(Settings::Instance().m_codec),
      m_refreshRate(Settings::Instance().m_refreshRate), m_renderWidth(width),
      m_renderHeight(height), m_bitrateInMBits(bitrate) {}

VideoEncoderNVENC::VideoEncoderNVENC(std::shared_ptr<CD3DRender> pD3DRender,
                                     int width, int height)
    : m_pD3DRender(pD3DRender), m_codec(Settings::Instance().m_codec),
      m_refreshRate(Settings::Instance().m_refreshRate), m_renderWidth(width),
      m_renderHeight(height), m_bitrateInMBits(30) {}
...
void VideoEncoderNVENC::Initialize() {
  ...
  FillEncodeConfig(initializeParams, m_refreshRate, m_renderWidth,
                   m_renderHeight, m_bitrateInMBits * 1'000'000L);
...
VideoEncoderNVENC::Transmit(ID3D11Texture2D *pTexture,
  ...
  if (params.updated) {
    ...
    NV_ENC_CONFIG encodeConfig = {NV_ENC_CONFIG_VER};
    initializeParams.encodeConfig = &encodeConfig;
    FillEncodeConfig(initializeParams, params.framerate, m_renderWidth,
                     m_renderHeight, m_bitrateInMBits * 1'000'000L);
...
void VideoEncoderNVENC::FillEncodeConfig(
    NV_ENC_INITIALIZE_PARAMS &initializeParams, int refreshRate,
    int renderWidth, int renderHeight, uint64_t bitrate_bps) {
  ...
  m_NvNecoder->CreateDefaultEncoderParams(&initializeParams, encoderGUID,
                                          qualityPreset, tuningPreset);
  ...
  // 16 is recommended when using reference frame invalidation. But it has
  // caused bad visual quality. Now, use 0 (use default).
  ...
  case ALVR_CODEC_H264: {
    ...
    if (Settings::Instance().m_nvencIntraRefreshPeriod != -1) {
      config.intraRefreshPeriod =
          Settings::Instance().m_nvencIntraRefreshPeriod;
    }
  ...
  // Disable automatic IDR insertion by NVENC. We need to manually insert IDR
  // when packet is dropped if don't use reference frame invalidation.
  ...
  } else if (Settings::Instance().m_nvencAdaptiveQuantizationMode ==
             TemporalAQ) {
  ...
  if (Settings::Instance().m_nvencRcInitialDelay != -1) {
    encodeConfig.rcParams.vbvInitialDelay =
        Settings::Instance().m_nvencRcInitialDelay;
  }
  ...
  if (Settings::Instance().m_nvencRcAverageBitrate != -1) {
    encodeConfig.rcParams.averageBitRate =
        Settings::Instance().m_nvencRcAverageBitrate;
  }
}
